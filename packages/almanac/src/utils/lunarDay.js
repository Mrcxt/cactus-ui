...
function e2c(yyyy, mm, dd) {
  TheDate = arguments.length != 3 ? new Date() : new Date(yyyy, mm, dd);
  debugger;
  let total, m, n, k;
  let isEnd = false;
...
function GetcDateString() {
  let tmp = "";
  const _tg = tgString.charAt((cYear - 4) % 10);
  const _dz = dzString.charAt((cYear - 4) % 12);
  const _sx = sx.charAt((cYear - 4) % 12);
...
export function GetLunarDay(yyyy, mm, dd) {
